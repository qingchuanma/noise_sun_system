...
uniform sampler2D rgbaNoiseTexture;
...
// void RGBAnoise(out vec4 fragColor, in vec2 fragCoord)
// {
...
float pn( in vec3 p )
{
    ...
    uv = textureLod( rgbaNoiseTexture, (uv+ 0.5)/256.0, 0.0 ).yx; //!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!11
...
float SunSurface( in vec3 pos )
{
    ...
    r1.y = pos.y-0.1 - 0.7*h + 0.5*sin( 3.0*u_time+pos.x + 3.0*pos.z);
...
float map(vec3 p) {
   p.z += 1.;
   R(p.yz, -0.01);
   R(p.xz, 1.0*0.008*pi+u_time*0.07);
   return (SunSurface(p * 1.777) +  (fpn(p*2.777+u_time*1.777) * 4.777) -  (fpn(p*100.+u_time*-70.) * 0.5) -1.0) * 0.777;
}
...
{  
   ...
   vec3 rd = normalize(vec3((vPosition.xy-0.5*u_resolution.xy)/u_resolution.y, 1.));
   vec3 ro = vec3(0., 0., -22.);
   ...
   #ifdef DITHERING
   vec2 pos = ( vPosition.xy / u_resolution.xy );
   vec2 seed = pos + fract(u_time);
   t=(1.+0.2*rand(seed));
   #endif
   ...
   vec4 fragColor = vec4(sqrt(tc), 1.0);
